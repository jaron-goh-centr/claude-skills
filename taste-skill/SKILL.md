...
## PREREQUISITES — invoke before reading further

**Step 1 — MANDATORY:** Invoke `soft-skill` now (via the Skill tool). This must always fire before any taste-skill work begins. No exceptions.

**Step 2 — EVALUATE & INVOKE if applicable:**
- **GSAP skills** — invoke the relevant subset if the task involves scroll-linked animation, complex motion timelines, per-element stagger, or pinned scroll sequences:
  - `gsap-scrolltrigger` → scroll-linked / pinned sections
  - `gsap-timeline` → sequenced multi-step animation
  - `gsap-react` → GSAP in React/Next.js
  - `gsap-core` → base API (to/from/fromTo, easing)
  - `gsap-plugins` → SplitText, Flip, Draggable, etc.
- **`webgpu-threejs-tsl`** — invoke if the task involves GPU-rendered visuals, particle systems, 3D scenes, shader effects, or WebGL/Three.js.

Only invoke what the task genuinely requires. If the design is static or uses only CSS/Framer Motion, skip the GSAP/WebGPU skills.
