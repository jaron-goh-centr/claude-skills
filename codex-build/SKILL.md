...
- Codex authenticated (prior `codex login`; ChatGPT account is fine). On a build/logic failure, surface it — don't silently retry. On an *auth or rate-limit* error specifically, one fallback is allowed: see "CLIProxyAPI fallback" below.
- **CLIProxyAPI fallback (optional, resilience only):** if a local CLIProxyAPI instance is running (`curl -sf http://localhost:8317/v1/models`) and the primary `codex exec` call fails with an auth or rate-limit error, retry the SAME call once with `--profile cliproxy` appended before surfacing the failure to the user. This routes through `~/.codex/cliproxy.config.toml`, a second Codex OAuth credential held by the proxy, independent of the primary `codex login` session. If the proxy isn't running, skip this and fail as before — never block waiting on it. Note in `LOG_FILE` which path succeeded (direct vs. proxy).
...
- `2>/dev/null` suppresses cosmetic MCP/auth stderr noise. Confirm success by the report file + a `thread.started` line; neither → failed run. If the failure looks like auth/rate-limit (not a task/logic error), retry once with `--profile cliproxy` appended (see "CLIProxyAPI fallback" in Prerequisites) before stopping and telling the user.
...
Re-verify (Step 3) after each round. If a resume call fails on auth/rate-limit, retry once with `--profile cliproxy` appended before counting the round as failed. After `MAX_FIX_ROUNDS` failed rounds: STOP delegating — Claude takes over and finishes the remaining fixes directly. Log the takeover. Ping-ponging trivia through delegation burns more than it saves.
